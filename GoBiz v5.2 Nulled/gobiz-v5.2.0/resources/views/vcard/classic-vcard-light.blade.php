...
<head>
    ...
    <meta name="theme-color" content="{{ $business_card_details->theme_color }}" />
...
<body class="antialiased bg-body text-body font-body" dir="{{(App::isLocale('ar') || App::isLocale('ur') || App::isLocale('he') ? 'rtl' : 'ltr')}}">
    <div>
        @if ($business_card_details != null)
            <div id="profile"
                ...
                <div class="max-w-xl w-full mx-auto bg-white rounded-lg overflow-hidden shadow-lg">
                    ...
                    <ul class="grid grid-flow-col grid-cols-1 grid-rows-1">
                        ...
                        @if ($feature->type == 'wa')
                        <a class="break-word" href="http://wa.me/{{ $feature->content }}" rel="noopener nofollow noreferrer" target="_blank">
                        @endif

                        @if ($feature->type == 'url')
                        <a class="break-all" href="https://{{ str_replace('https://', '', $feature->content) }}" rel="noopener nofollow noreferrer" target="_blank">
                        @endif
                        ...
                        <li>
                            <div class="flex items-center w-full px-5 ">
                                <div
                                    class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
                        ...
                        @foreach ($service_details as $service_detail)
                            <div class="mb-3">
                                <div class="w-full overflow-hidden rounded-lg shadow-lg">
                                    <a href="{{ asset($service_detail->service_image) }}" data-toggle="lightbox" data-gallery="gallery" class="col-md-4">
                                        <img class="w-full"
                            ...
                            <div class="p-3 bg-white shadow-lg rounded-lg">
                                <div class="w-full mb-2">
                                    <img class="rounded pb-2"
                                        ...
                                        src="{{ asset($product_detail->product_image) }}" alt="{{ $product_detail->product_name }}">
                                </div>
                                ...
                                <div class="w-full mb-1 mt-1 justify-between items-center">
                                    <div>
                                        <h3 id="{{ $product_detail->product_id }}_product_name" class="text-sm font-medium text-dark">
                                            {{ $product_detail->product_name }}</h3>
                                        <span id="{{ $product_detail->product_id }}_subtitle" class="text-xs text-dark">{{
                                            $product_detail->product_subtitle }}</span>
                                    </div>
                                </div>
                                <div class="w-full mb-1 justify-between items-center">
                                    <h4 class="text-sm mb-3 font-bold text-dark"><span id="{{ $product_detail->product_id }}_currency">{{ $product_detail->currency }}</span> <span id="{{ $product_detail->product_id }}_price">{{
                                            $product_detail->sales_price }}</span>
                                        @if ($product_detail->sales_price != $product_detail->regular_price)
                                        <span class="text-xs line-through text-red-500 font-bold">
                                            {{ $product_detail->currency }} {{ $product_detail->regular_price }}</span>
                                        @endif
                                    </h4>
                                    @if ($enquiry_button != null)
                    ...
                    <div class="w-full border-t px-5 align-middle py-4 border-b">
                        ...
                        <div id="gallery" class="px-5 py-4">
                            @foreach ($galleries_details as $galleries_detail)
                                <div class="mb-3">
                                    <div class="w-full overflow-hidden rounded-lg shadow-lg">
                                        <a href="{{ asset($galleries_detail->gallery_image) }}" data-toggle="lightbox" data-gallery="gallery" class="col-md-4">
                                            <img class="w-full"
                    ...
                    @if ($feature_details != null && !$feature_details->isEmpty())
                        <div id="youtube" class="px-5 py-4">
                            @foreach ($feature_details as $feature)
                                ...
                                <div class="w-full border-t px-5 align-middle py-4 border-b">
                                    <p class="text-gray-700 font-semibold text-lg">{{ __('Youtube Videos') }}</p>
                                </div>
                                <div class="mb-3">
                                    <div class="w-full overflow-hidden rounded-lg shadow-lg">
                    ...
                    @if (count($payment_details) > 0)
                        ...
                        @foreach ($payment_details as $payment)
                            ...
                            <a href="{{ str_replace('https://', '', $payment->content) }}" rel="noopener nofollow noreferrer" target="_blank">
                            @else
                            <a href="https://{{ str_replace('https://', '', $payment->content) }}" rel="noopener nofollow noreferrer" target="_blank">
                            @endif
                            @endif
                    ...
                    @if ($business_hours != null && $business_hours->is_display != 0)
                        ...
                        <div class="px-5 py-4">
                            ...
                            @if ($business_hours->is_always_open != 'Opening')
                                <div>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Monday') }}: {{ __($business_hours->Monday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Tuesday') }}: {{ __($business_hours->Tuesday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Wednesday') }}: {{ __($business_hours->Wednesday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Thursday') }}: {{ __($business_hours->Thursday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Friday') }}: {{ __($business_hours->Friday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Saturday') }}: {{ __($business_hours->Saturday) }}</p>
                                    <p class="pt-2 font-semibold text-gray-800">{{ __('Sunday') }}: {{ __($business_hours->Sunday) }}</p>
                                </div>
                            @else
                                <div>
                        ...
                        <div class="container mx-auto px-4">
                          <div class="max-w-xl mx-auto">
                            <div class="">
                              ...
                              <div class="bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 border-t-4 border-{{ $business_card_details->theme_color }}-500 rounded-b text-{{ $business_card_details->theme_color }}-900 px-4 py-3 shadow-md mb-3" role="alert">
                                <div class="flex">
                                  ...
                                  <div>
                                    <p class="font-bold text-white">{{ Session::get('message') }}</p>
                                    <p class="text-sm text-white">{{ __('Please wait for the reply to be sent.') }}</p>
                                  </div>
                                </div>
                              </div>
                              @endif
                              <form action="{{ route('sent.enquiry') }}" method="POST">
                                @csrf
                                <div class="flex flex-wrap -mx-2">
                                  <div class="mb-3 w-full lg:w-1/2 px-2">
                                    <input
                                      class="w-full p-4 text-xs bg-gray-50 outline-none rounded"
                                      type="hidden" value="{{ $business_card_details->card_id }}" name="card_id"
                                    />
                                    <input
                                ...
                                <div class="mb-3 flex p-4 bg-gray-100 rounded">
                                  <input
                                    class="w-full text-xs bg-gray-100 outline-none"
                                    type="number"
                                    placeholder="{{ __('Mobile Number') }}" name="phone"
                                  />
                                </div>
                                <div class="mb-6 flex p-4 bg-gray-100 rounded">
                                  <textarea
                                    class="w-full h-20 text-xs font-semibold bg-gray-100 rounded outline-none"
                                    type="text"
                                    placeholder="{{ __('Message') }} *" name="message" required
                                  ></textarea>
                                </div>
                                <div class="text-center">
                                  <button
                                    class="mb-2 w-full py-4 bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 text-sm rounded-lg font-bold text-gray-50 transition duration-200"
                                  >
                                    {{ __('Send')}}
                                  </button>
                                </div>
                    ...
                    <div class="lg:block hidden w-full ml-6 pb-3 mx-auto border-t pt-3">
                        <ul class="grid grid-flow-col grid-cols-6 grid-rows-1">
                            ...
                            <li class="flex send-modal-open cursor-pointer items-center">
                                <div
                                    class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-md hover:shadow-lg h-16 w-16 rounded-lg fill-current text-white">
                            ...
                            <li class="flex modal-open cursor-pointer items-center">
                                <div
                                    class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-16 w-16 rounded-lg fill-current text-white">
                    ...
                    <div id="share" class="w-full border-t px-5 align-middle py-4 border-b">
                        ...
                        <div class="w-full ml-6 pb-3 mx-auto border-t pt-3">
                            <ul class="grid grid-flow-col lg:grid-cols-8 grid-cols-6 grid-rows-1">
                                ...
                                <a target="_blank" rel="noopener nofollow noreferrer" href="{{ $shareComponent['facebook'] }}">
                                 ...
                                 <a target="_blank" rel="noopener nofollow noreferrer" href="{{ $shareComponent['twitter'] }}">
                                    <li class="flex cursor-pointer items-center">
                                        <div
                                            class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
                                 ...
                                 <a target="_blank" rel="noopener nofollow noreferrer" href="{{ $shareComponent['linkedin'] }}">
                                    <li class="flex cursor-pointer items-center">
                                        <div
                                            class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
                                 ...
                                 <a target="_blank" rel="noopener nofollow noreferrer" href="{{ $shareComponent['telegram'] }}">
                                    <li class="flex cursor-pointer items-center">
                                        <div
                                            class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
                                 ...
                                 <a target="_blank" rel="noopener nofollow noreferrer" href="{{ $shareComponent['whatsapp'] }}">
                                    <li class="flex cursor-pointer items-center">
                                        <div
                                            class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-lg hover:shadow-lg h-12 w-12 rounded-lg fill-current text-white">
            ...
            {{-- Save Contact --}}
            <div class="lg:hidden bg-{{ $business_card_details->theme_color }}-800 fixed bottom-0 w-full border-t border-gray-200 flex">
                {{-- Scan QR --}}
                <a href="#"
                    class="flex modal-open flex-grow items-center justify-center p-2 text-gray-50 hover:text-gray-50 bg-{{ $business_card_details->theme_color }}-800">
                    <div class="text-center">
                ...
                {{-- Download vCard --}}
                <a href="{{ route('download.vCard', $business_card_details->card_id) }}"
                    class="flex flex-grow items-center justify-center p-2 text-gray-50 hover:text-gray-50 bg-{{ $business_card_details->theme_color }}-800">
                    <div class="text-center">
                        <span class="block h-8 grid justify-items-center text-3xl leading-8">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                    ...
                    <div class="modal-content py-4 text-left px-6">
                        <div class="justify-between items-center">
                            <div class="text-left my-3">
                                <label class="mt-6 block text-gray-700 text-sm font-bold mb-2" for="phone_number">{{ __("Phone Number") }}</label>
                                <input id="phone_number" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" placeholder="{{ __('For ex: 91987654310') }}">
                                <small>{{ __('For ex: 91987654310 (With Country code) (Without +)') }}</small>
                            </div>
                            <button class="flex justify-center items-center content-center bg-gradient-to-br from-{{ $business_card_details->theme_color }}-300 to-{{ $business_card_details->theme_color }}-800 shadow-md hover:shadow-lg h-14 w-12 py-2 px-8 fill-current text-white" onclick="sendVcard()">
                                {{ __('Send') }}
                            </button>
                        </div>
                    </div>
